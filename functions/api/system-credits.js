...
export async function onRequest(context) {
    ...
    try {
        ...
        const { max_tokens: rawMaxTokens, ...upstreamRequestBody } = requestBody;
        const requestedMaxTokens = Number(rawMaxTokens);
        const hasExplicitMaxTokens = Number.isFinite(requestedMaxTokens) && requestedMaxTokens > 0;
        const openaiBody = {
            ...upstreamRequestBody,
            model: selectedModel,
            stream: stream,
            ...(hasExplicitMaxTokens && { max_tokens: Math.floor(requestedMaxTokens) }),

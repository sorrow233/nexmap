...
// api/redeem.js
async function verifyFirebaseToken2(token) {
  if (!token) return null;
  try {
    const parts = token.split(".");
    if (parts.length !== 3) return null;
...
__name(verifyFirebaseToken2, "verifyFirebaseToken");
async function onRequest6(context) {
  ...
  try {
    let getCurrentWeekNumber2 = function() {
    ...
    __name(getCurrentWeekNumber2, "getCurrentWeekNumber");
    const authHeader = request.headers.get("Authorization");
    const token = authHeader?.replace("Bearer ", "");
    const userId = await verifyFirebaseToken2(token);
    if (!userId) {
...
async function verifyFirebaseToken3(token) {
  if (!token) return null;
  try {
    const parts = token.split(".");
    if (parts.length !== 3) return null;
...
__name(verifyFirebaseToken3, "verifyFirebaseToken");
...
async function onRequest7(context) {
  ...
  try {
    const authHeader = request.headers.get("Authorization");
    const token = authHeader?.replace("Bearer ", "");
    const userId = await verifyFirebaseToken3(token);
    if (!userId) {
      return new Response(JSON.stringify({
        error: "Unauthorized",
...
// ../.wrangler/tmp/pages-IVtYOQ/functionsRoutes-0.09596630995356659.mjs
var routes = [
  {

...
var SUPPORT_EMAIL = "support.nexmap@catzz.work";
...
async function onRequestGet(context) {
  ...
  try {
    const authHeader = request.headers.get("Authorization");
    const token = authHeader?.replace("Bearer ", "");
    const userId = await verifyToken2(token);
    if (!userId) {
    ...
    const stripeKey = env.STRIPE_SECRET_KEY;
    if (!stripeKey) throw new Error("Stripe key not configured");
    const stripeRes = await fetch(`${STRIPE_API2}/checkout/sessions/${sessionId}`, {
      headers: {
        "Authorization": `Bearer ${stripeKey}`
      }
    });
...
// api/gmi-serving.js
var THINKING_LEVEL_ALLOWLIST = /* @__PURE__ */ new Set(["THINKING_LEVEL_UNSPECIFIED", "LOW", "HIGH"]);
var RETRYABLE_STATUS_CODES = /* @__PURE__ */ new Set([408, 409, 425, 429, 500, 502, 503, 504, 524]);
...
function computeBackoffDelay(attempt, baseMs = 700, maxMs = 8e3) {
...
__name(isRetryableText, "isRetryableText");
...
__name(shouldRetryUpstream, "shouldRetryUpstream");
async function fetchUpstreamWithRetry(url, requestInit, { stream = false } = {}) {
  const maxAttempts = 3;
  const timeoutMs = stream ? 95e3 : 45e3;
  let attempt = 0;
  let lastError = null;
...
__name(fetchUpstreamWithRetry, "fetchUpstreamWithRetry");
...
__name(normalizeThinkingLevelInRequest, "normalizeThinkingLevelInRequest");
...
// ../.wrangler/tmp/pages-DNVyrG/functionsRoutes-0.6431411668421584.mjs
var routes = [
  {

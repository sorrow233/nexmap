...
const LanguageSwitcher = () => {
    ...
    return (
        ...
        >
            ...
            <div
                className={`
                    absolute top-full pt-2 w-32 rounded-xl transition-all duration-300 origin-top-right
                    ${isOpen ? 'opacity-100 scale-100 translate-y-0 pointer-events-auto' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'}
                `}
            >
                <div className="py-2 rounded-xl border border-white/10 bg-[#0A0A0A]/90 backdrop-blur-2xl shadow-2xl overflow-hidden">
                    {languages.map((lang) => (
                        <button
                            key={lang.code}
                            onClick={(e) => {
                                e.stopPropagation(); // Prevent parent click from immediately toggling

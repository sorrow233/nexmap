...
const S6_AutoSort = () => {
    ...
    const items = [
        { color: 'bg-blue-500', group: 'work', tx: -300, ty: -100 },
        { color: 'bg-blue-500', group: 'work', tx: -150, ty: -100 },
        { color: 'bg-emerald-500', group: 'personal', tx: 150, ty: -100 },
        { color: 'bg-emerald-500', group: 'personal', tx: 300, ty: -100 },
        { color: 'bg-purple-500', group: 'ideas', tx: -300, ty: 100 },
        { color: 'bg-purple-500', group: 'ideas', tx: -150, ty: 100 },
    ];
    ...
    // Observer to auto-trigger
    const TriggerRef = ({ onEnter, children }) => {
        const ref = React.useRef();
        React.useEffect(() => {
            const observer = new IntersectionObserver(([entry]) => {
                if (entry.isIntersecting && !sorted) onEnter();
            }, { threshold: 0.5 });
        ...
        <section className="h-screen w-full bg-zinc-900 relative flex flex-col items-center justify-center overflow-hidden">
            <TriggerRef onEnter={() => setTimeout(toggleSort, 500)}>
                <div className="absolute inset-0 flex items-center justify-center">
                    {/* Zones Background (Only visible when sorted) */}
                    ...
                    {springs.map((props, i) => (
                        <animated.div
                            key={i}
                            style={props}
                            className={`absolute w-32 h-20 rounded-lg shadow-lg border border-white/10 ${items[i].color}`}
                        />
                    ))}
                </div>

                <div className="relative z-10 text-center mt-[40vh] pointer-events-none">
                    <h2 className="text-5xl font-bold text-white mb-2">Chaos to Order</h2>
                    <p className="text-white/60">One click organization.</p>
                </div>

                <button
                    onClick={toggleSort}
                    className="absolute bottom-20 px-8 py-3 bg-white text-black rounded-full font-bold hover:scale-105 transition-transform z-20"
                >
                    {sorted ? 'Scramble' : 'Organize'}
                </button>
            </TriggerRef>
        </section>
    );

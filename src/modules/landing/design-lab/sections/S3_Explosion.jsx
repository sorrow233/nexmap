                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    <div className="relative z-50 text-center pointer-events-none mix-blend-difference">
                        <animated.h2
                            style={{
                                opacity: expansion,
                                transform: expansion.to(e => `scale(${0.5 + e * 0.5})`)
                            }}
                    ...
                    {/* Exploding Particles/Cards */}
                    {[...Array(12)].map((_, i) => (
                        <animated.div
                            key={i}
                            className="absolute w-32 h-20 bg-gradient-to-br from-blue-600 to-purple-600 rounded-lg shadow-2xl border border-white/20"
                            style={{
                                opacity: expansion,
                                transform: expansion.to(e => {
                                    const angle = (i / 12) * Math.PI * 2;
                                    const radius = 100 + e * 600; // Fly out to 700px
                    ...
                    {/* Shockwave */}
                    <animated.div
                        className="absolute w-full h-full border-4 border-white rounded-full bg-blue-500/20"
                        style={{
                            opacity: expansion.to(e => 1 - e),
                            transform: expansion.to(e => `scale(${e * 3})`)
                        }}
                    />
                </div>
            </TriggerRef>

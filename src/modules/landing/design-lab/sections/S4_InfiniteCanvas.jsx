...
const S4_InfiniteCanvas = () => {
    ...
    // Auto-pan effect or scroll-driven pan
    useEffect(() => {
        const handleScroll = () => {
            if (!containerRef.current) return;
            const rect = containerRef.current.getBoundingClientRect();
            // Map scroll position to x/y offset
            const scrollY = window.scrollY;
            setOffset({
                x: scrollY * 0.2, // Parallax X
                y: scrollY * 0.1  // Parallax Y
            });
        };
        ...
        <section ref={containerRef} className="h-screen w-full bg-[#111] overflow-hidden relative cursor-grab active:cursor-grabbing">
            ...
            <div className="absolute inset-0 pointer-events-none">
                {[...Array(8)].map((_, i) => (
                    <div
                        key={i}
                        className="absolute bg-zinc-800 border border-white/10 rounded-lg shadow-xl p-4 flex flex-col gap-2"
                        style={{
                            left: `${20 + (i * 15) + (Math.sin(i) * 10)}%`,
                            top: `${20 + (i * 10) + (Math.cos(i) * 20)}%`,
                            width: '200px',
                            height: '140px',

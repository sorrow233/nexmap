...
export class GeminiProvider extends LLMProvider {
    ...
    async chat(messages, model, options = {}) {
        ...
        while (retries >= 0) {
            try {
                console.log(`[Gemini] Sending chat request to /api/proxy for model ${cleanModel}`);
    ...
    async stream(messages, onToken, model, options = {}) {
        ...
        while (retries >= 0) {
            try {
                console.log(`[Gemini] Starting stream request to ${baseUrl} for model ${cleanModel} via /api/proxy`);
                const response = await fetch('/api/proxy', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },

    ...
    if (favorites.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-40 animate-fade-in text-slate-500">
                ...
                <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-2 font-inter-tight">{t.favorites.noFavorites}</h2>
                        ...
                        >
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="flex gap-1">
                                        <button
                                            ...
                                            title={t.favorites.shareTooltip}
                                        >
                                            <Share2 size={14} />
                                        </button>
                                        <button
                            ...
                            <div className="pt-4 border-t border-slate-100 dark:border-white/5 flex items-center justify-between mt-auto z-10">
                                <div className="flex flex-col">
                                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">{t.favorites.from}</span>
                    ...
                    <div className="w-full max-w-[1100px] h-full sm:h-[92vh] sm:rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] flex flex-col overflow-hidden animate-fade-in relative z-10 transition-all duration-500 bg-white/95 dark:bg-[#0A0A0A] backdrop-blur-2xl border border-slate-200 dark:border-white/10">
                        ...
                        <div className="h-20 px-10 flex justify-between items-center shrink-0 border-b border-slate-100 dark:border-white/5">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h3 className="font-bold text-slate-800 dark:text-slate-100 text-2xl tracking-tight leading-tight font-inter-tight">{t.favorites.favoriteNoteTitle}</h3>
                                    <span className="text-[10px] uppercase tracking-[0.2em] font-black text-indigo-500 dark:text-indigo-400">{t.favorites.collection}</span>
                                </div>
                            </div>
                        ...
                        <div className="px-10 py-4 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/50">
                            <div className="flex items-center justify-between text-xs">
                                <span className="text-slate-400 font-medium">{t.favorites.source}: <span className="font-bold text-slate-900 dark:text-white">{expandedFav.boardName}</span></span>
                                <button
                                ...
                                >
                                    {t.favorites.goToBoard} <ExternalLink size={12} />
                                </button>
                            </div>
                        </div>
                    </div>

...
import { RefreshCw, FileX2 } from 'lucide-react';
        ...
        <>
            ...
            <section className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    <h2 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white shrink-0">{labels.title}</h2>
                    <NotesStatsPanel stats={stats} labels={labels} />
                </div>

                <button
                    onClick={() => refreshNotes({ background: true })}
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-white/10 dark:text-slate-200 dark:hover:bg-white/15 transition-colors shrink-0"
                >
                    <RefreshCw size={14} className={isRefreshing ? 'animate-spin' : ''} />
                    {isRefreshing ? labels.syncing : labels.refresh}
                </button>
            </section>

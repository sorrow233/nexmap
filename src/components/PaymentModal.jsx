                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/30 rounded-xl p-4">
                                <div className="flex items-center justify-between flex-wrap gap-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm text-white/80">{t.pricing.youGet} <b className="text-emerald-400">200 {t.pricing.chatsPerWeek}</b> + <b className="text-emerald-400">20 {t.pricing.images}</b> {t.pricing.perWeek}</span>
                                    </div>
                                    <span className="text-xs text-white/40">{t.pricing.resetsMonday}</span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <PlanCard
                                    ...
                                    amount="$0.99"
                                    chats={`600 ${t.pricing.conversations}`}
                                    features={[t.pricing.payAsYouGo, t.pricing.noExpiration]}
                                    onClick={() => handleCheckout('credits_500')}
                                    loading={loadingProduct === 'credits_500'}
                                    btnText={t.common.buy || 'Buy'}
                                />
                                {/* Standard - 3,000 chats (Recommended) */}
                                <PlanCard
                                    title={t.pricing.standard}
                                    amount="$3.99"
                                    chats={`3,000 ${t.pricing.conversations}`}
                                    popular
                                    recommendedText={t.pricing.mostPopular}
                                    features={[t.pricing.bestValue, t.pricing.prioritySupport]}
                                ...
                                {/* Power - 9,000 chats */}
                                <PlanCard
                                    title={t.pricing.power}
                                    amount="$9.99"
                                    chats={`9,000 ${t.pricing.conversations}`}
                                    features={[t.pricing.forPowerUsers, t.pricing.maxEfficiency]}
                        ...
                        <div className="bg-gradient-to-br from-amber-500/10 to-orange-500/5 rounded-2xl p-1 border border-amber-500/20">
                            <div className="flex flex-col md:flex-row gap-8 p-6 items-center">
                                <div className="flex-1 space-y-6">
                                    <div>
                                        <h3 className="text-2xl font-bold text-white mb-2">{t.pricing.becomePro}</h3>
                                        <p className="text-amber-200/80">{t.pricing.unlockPotential}</p>
                                    </div>
                                    <ul className="space-y-3">
                                        <li className="flex gap-3 text-slate-300 text-sm">
                                            <Check className="text-amber-500 shrink-0" size={18} />
                                            <span>{t.pricing.ownKey} (OpenAI, Anthropic, Google)</span>
                                        </li>
                                        <li className="flex gap-3 text-slate-300 text-sm">
                                            <Check className="text-amber-500 shrink-0" size={18} />
                                ...
                                <div className="w-full md:w-auto min-w-[200px] text-center bg-black/20 p-6 rounded-xl border border-white/5">
                                    <div className="text-sm text-slate-400 mb-1">{t.pricing.oneTimePayment}</div>
                                    ...
                                    >
                                        {loadingProduct === 'pro_lifetime' ? t.pricing.processing : t.pricing.upgradeNow}
                                    </button>
                                </div>
                            </div>
                        </div>
...
const PlanCard = ({ title, amount, chats, popular, features, onClick, loading, recommendedText, btnText }) => (

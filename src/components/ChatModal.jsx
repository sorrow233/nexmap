...
export default function ChatModal({ card, isOpen, onClose, onUpdate, onGenerateResponse }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" style={{ perspective: '1000px', fontFamily: '"LXGW WenKai", "楷体", "KaiTi", serif' }}>
            ...
            <div className="bg-slate-900/90 backdrop-blur-2xl w-[900px] max-w-full h-[85vh] rounded-3xl shadow-2xl flex flex-col border border-white/10 overflow-hidden animate-fade-in relative z-10 font-sans">

                {/* Header */}
                <div className="h-20 px-8 border-b border-white/5 flex justify-between items-center bg-slate-900/40 shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="font-bold text-slate-100 text-xl tracking-tight leading-tight">
                                {card.data.title || 'New Conversation'}
                            </h3>
                            <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-brand-400/60 mt-0.5">Neural Canvas Mode</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        className="w-10 h-10 rounded-full flex items-center justify-center text-slate-500 hover:bg-white/5 hover:text-white transition-all active:scale-90"
                    >
                        <X size={22} />
                    </button>
                </div>
                ...
                {/* Messages Area */}
                <div className="flex-grow overflow-y-auto p-8 space-y-8 custom-scrollbar bg-slate-950/30">
                        ...
                        return (
                            <div key={i} className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-slide-up group`}>
                                <div className={`
                                    ...
                                    p-5 sm:p-6 
                                    rounded-[2rem] shadow-[0_4px_20px_rgba(0,0,0,0.2)] text-sm sm:text-base leading-relaxed
                                    ${isUser
                                ...
                                `}>
                                    {thoughts && (
                                        <details className="mb-4 group/think">
                                            <summary className="text-xs font-bold text-brand-400/50 cursor-pointer list-none flex items-center gap-2 hover:text-brand-400 transition-all uppercase tracking-widest">
                                            ...
                                            <div className="mt-3 p-4 bg-slate-900/50 rounded-2xl text-xs font-mono text-slate-400 whitespace-pre-wrap border border-white/5 leading-relaxed italic">
                                                {thoughts}
                                            </div>
                                        </details>
                                    )}
                                    <div
                                        className={`prose max-w-none text-sm sm:text-[15px] font-lxgw ${isUser ? 'user-bubble prose-invert' : 'prose-invert'}`} // prose-invert forces dark mode text
                    ...
                    {isStreaming && (
                        <div className="flex justify-start animate-pulse">
                            <div className="bg-brand-900/20 px-5 py-2.5 rounded-full text-xs font-bold text-brand-400 border border-brand-500/20 flex items-center gap-2">
                                <div className="flex gap-1">
                                    <span className="w-1 h-1 bg-brand-400 rounded-full animate-bounce"></span>
                                    <span className="w-1 h-1 bg-brand-400 rounded-full animate-bounce [animation-delay:0.2s]"></span>
                                    <span className="w-1 h-1 bg-brand-400 rounded-full animate-bounce [animation-delay:0.4s]"></span>
                                </div>
                                AI is composing...
                            </div>
                        </div>
                ...
                {/* Input Area */}
                <div className="p-4 sm:p-6 bg-slate-900/60 border-t border-white/5 backdrop-blur-md">
                    <div className="relative group">
                        <textarea
                            value={input}
                            onChange={e => setInput(e.target.value)}
                            onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                            className="w-full p-4 pr-14 bg-slate-800/50 border border-white/10 rounded-2xl focus:bg-slate-800 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500/50 transition-all outline-none resize-none h-[80px] text-slate-200 placeholder:text-slate-500"
                    ...
                    <div className="text-center mt-2">
                        <span className="text-[10px] uppercase tracking-widest text-slate-600 font-semibold">{card.data.model || 'AI Model'}</span>
                    </div>
                </div>
            </div>
        </div>

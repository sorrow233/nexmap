/**
 * 简体中文翻译
 */
export const zh = {
    nav: {
        about: "关于",
        history: "历史",
        pricing: "定价"
    },
    hero: {
        brand: "NexMap 2.0",
        title1: "无限",
        title2: "画布。",
        sub1: "忘掉文件，打破层级。",
        sub2: "让一切在「连接」中重生。",
        ctaPrimary: "开启无限思维",
        ctaSecondary: "见证奇迹",
        cards: {
            alpha: "Project Alpha",
            ideas: "灵感库",
            tags: "标签流",
            explore: "探索边界"
        }
    },
    bento: {
        headline: {
            pre: "为",
            highlight: "专业",
            post: "LLM 用户打造。"
        },
        subtext: "为了构建终极引擎，我们解锁了无上限并发以支持海量并行任务，通过递归图遍历器深度解析上下文，并实现了城市级的空间分区架构。",
        graph: {
            badge: "图上下文游走",
            title: "它能读懂连接。",
            text: "传统的聊天界面对结构一无所知。我们的引擎会遍历你画布上的语义结构图，修剪不相关的节点，并将精确的相邻上下文注入到每一次生成中。",
            stat1: "Token 窗口",
            stat2: "深度"
        },
        sprout: {
            title: "递归“发芽”",
            text: "主动构思。点击“发芽”，让 AI 递归地将一个想法分支成五条不同的执行路径。"
        },
        concurrency: {
            title: "无限并发",
            text: "没有“思考中…”的阻碍。同时启动 50 个流。我们的非阻塞 AIManager 轻松应对负载。"
        },
        spatial: {
            badge: "空间分区",
            title: "语义分区。",
            text: "不仅仅是组织——而是建造城市。将相关的想法分组到动态的“区域”中，这些区域会自动调整其边界框。就像神经架构的“城市：天际线”。",
            zoneExample: "区域：架构",
            autoExpand: "自动扩展"
        }
    },
    concurrency: {
        badge: "非阻塞式架构",
        title: "无限并发。",
        text: "告别“正在思考”的等待。50+ 线程同时迸发，非阻塞架构让你的灵感无需排队，火力全开。",
        idle: "待机中..."
    },
    spatial: {
        title1: "空间化",
        title2: "组织架构。",
        text: "思维特区。不仅仅是整理，而是在规划你的思维城市。创建动态调整的“特区 (Zones)”，让混乱的想法自动归位，井然有序。",
        stateOrganized: "状态: 有序",
        stateChaos: "状态: 混沌",
        zones: {
            arch: "特区: 架构设计",
            eng: "特区: 工程实现"
        },
        cards: {
            floorPlans: "平面图",
            elevations: "立面图",
            materials: "材质库",
            structures: "结构分析",
            electrical: "电气系统"
        }
    },
    sprout: {
        badge1: "128k 上下文窗口",
        badge2: "∞ 探索深度",
        title1: "递归式",
        title2: "“思维裂变”",
        text: "拒绝单线程。点击“Sprout”，让一个念头瞬间衍生出五条并行的思考路径，灵感从此指数级爆发。"
    },
    graph: {
        badge: "引擎核心 v2.1",
        title1: "图谱语境",
        title2: "深度游走。",
        text: "洞察每一条脉络。传统对话是线性的，而思维是网状的。Mixboard 能够遍历你画布上的语义网络，修剪无关枝节，让每一次生成都精准切中上下文要害。"
    },
    demoInfinite: {
        title: "秩序之美。",
        text: "观看 AI 扫描你零散的想法，并将它们瞬间整理得井井有条。",
        organizing: "AI 正在重构...",
        complete: "结构构建完成",
        cards: {
            roadmap: "路线图",
            planning: "全盘规划",
            assets: "核心资产",
            design: "设计规范",
            schema: "数据模型",
            dev: "开发文档",
            ideas: "灵感碎片",
            brainstorm: "头脑风暴",
            sources: "数据源",
            research: "深度调研",
            sprints: "冲刺计划",
            agile: "敏捷迭代"
        }
    },
    sidebar: {
        global: "全局",
        board: "当前画布",
        myPrompts: "我的提示词",
        addPrompt: "新建提示词",
        save: "保存",
        cancel: "取消",
        deleteConfirm: "确定删除此提示词吗？",
        instruction: "指令",
        dragHelp: "拖拽至对话框或画布"
    },
    settings: {
        title: "设置",
        configuration: "配置",
        general: "通用",
        language: "语言",
        credits: "额度",
        creditsDesc: "管理使用量与限制",
        advancedSettings: "高级设置",
        aiConfiguration: "AI 配置",
        provider: "模型提供商",
        providerDesc: "模型 & API 密钥",
        modelRoles: "模型角色",
        modelRolesDesc: "分配专用模型",
        dataStorage: "数据 & 存储",
        storage: "存储",
        storageDesc: "S3 & 云端设置",
        cancel: "取消",
        saveChanges: "保存更改",
        creditsUsage: "额度 & 用量",
        modelProvider: "模型提供商",
        storageSettings: "存储设置",
        settingsSaved: "设置已保存！",
        applyingChanges: "正在应用更改...",
        resetConfiguration: "重置配置？",
        resetWarning: "这将删除所有自定义提供商和 API 密钥。应用将恢复到默认状态。",
        yesReset: "确认重置",
        customInstructions: "自定义指令",
        customInstructionsDesc: "全局 AI 行为",
        customInstructionsHelp: "在此添加的指令将应用于所有卡片和画布中的每次 AI 交互。",
        customInstructionsPlaceholder: "示例：请用轻松友好的语气回复。使用项目符号列表。回答尽量简洁。",
        customInstructionsNote: "工作原理",
        customInstructionsInfo: "您的指令会被添加到每个 AI 请求的开头。用于设置语言偏好、回复风格或特定领域的上下文。",
        exampleInstructions: "示例指令",
        exampleInstruction1: "请始终用日语回复",
        exampleInstruction2: "我是软件工程师，请使用专业术语",
        exampleInstruction3: "回复控制在 200 字以内",
        connectionSuccess: "连接成功！",
        connectionFailed: "连接失败",
        testConnection: "测试连接",
        testing: "测试中...",
        resetDefaults: "重置默认",
        newProvider: "新建提供商",
        providerName: "提供商名称",
        apiKey: "API 密钥",
        baseUrl: "Base URL",
        modelName: "模型名称",
        protocol: "协议",
        geminiNative: "Gemini 原生",
        openaiCompat: "OpenAI 兼容",
        geminiKeyPlaceholder: "Gemini API Key",
        openaiKeyPlaceholder: "sk-...",
        urlPlaceholder: "https://api.openai.com/v1",
        modelPlaceholder: "gpt-4o",
        exampleUrlGemini: "示例: https://generativelanguage.googleapis.com/v1beta",
        exampleUrlOpenai: "示例: https://api.openai.com/v1",
        languageChoose: "选择界面显示语言。",
        languageNote: "注意：",
        languageNoteDesc: "切换语言将立即更新用户界面。部分 AI 生成的内容（如现有的卡片）将保持其原始语言。",
        roles: {
            title: "模型分配",
            description: "为不同功能分配特定模型。这些设置仅适用于",
            chatTitle: "对话模型",
            chatDesc: "用于卡片对话的主模型",
            analysisTitle: "想法发芽 (分析)",
            analysisDesc: "用于生成后续问题的模型",
            imageTitle: "图片生成",
            imageDesc: "用于创建画板背景的模型",
            important: "重要",
            importantText: "这些角色分配是针对每个提供商单独保存的。当您切换提供商时，系统会自动切换到为该提供商配置的角色。"
        },
        storageConfig: {
            byok: "BYOK (自带密钥)",
            byokDesc: "使用您自己的 S3 存储 (AWS, Cloudflare R2, MinIO) 来存储图片。",
            enable: "启用 S3 存储",
            enableDesc: "将图片上传到您自己的云端存储桶",
            endpoint: "Endpoint URL",
            region: "Region (区域)",
            bucket: "Bucket Name (存储桶名)",
            accessKey: "Access Key ID",
            secretKey: "Secret Access Key",
            recovery: "数据恢复",
            backupFound: "发现安全备份",
            backupFoundDesc: "我们发现了上次注销前创建的本地备份。您可以尝试将此数据恢复到您的云端账户。",
            restoreComplete: "恢复完成！",
            restore: "恢复备份",
            restoring: "正在恢复...",
            noBackup: "未发现待处理的安全备份。",
            advancedRecovery: "显示高级恢复 (手动导入)",
            hideAdvancedRecovery: "隐藏高级恢复",
            manualImport: "手动 JSON 导入",
            manualImportDesc: "请在下方粘贴支持团队提供的原始备份数据 JSON。",
            importRestore: "导入并恢复",
            importing: "正在导入...",
            scheduledBackups: "计划备份",
            scheduledDesc: "每天凌晨 3:00 和下午 4:00 自动备份 (保留5天历史)",
            backupNow: "立即备份",
            backingUp: "正在备份...",
            nextBackup: "下次备份：",
            noBackupsYet: "暂无备份。备份将在预定时间自动创建。"
        },
        app: {
            settings: "应用程序设置",
            showWelcome: "显示欢迎屏幕",
            showWelcomeDesc: "再次查看介绍和指南",
            show: "显示欢迎页"
        }
    },
    credits: {
        noConfigNeeded: "无需配置，直接使用",
        readyToUse: "我们已经为您预置了最佳 AI 模型。您现在的免费额度足够支持约",
        interactions: "200",
        conversations: "次对话。",
        remainingCredits: "剩余额度",
        imageCredits: "图片生成",
        fastResponse: "极速响应",
        fastResponseDesc: "DeepSeek V3 高性能模型",
        longLasting: "持久续航",
        longLastingDesc: "无需担心额度耗尽",
        advancedNote: "*如需使用您自己的 API Key (OpenAI, Google, Anthropic)，请前往",
        advancedLink: "高级设置",
        toConfig: "进行配置。",
        startExploring: "开始探索",
        poweredBy: "Powered by DeepSeek V3.2",
        welcomeTitle: "无需配置，即刻出发",
        initialCredits: "初始额度可进行约",
        plus: "+",
        noConfigDesc: "你无需配置一切，我已为你想好一切。",
        startJourney: "请立即开始你的卡片之旅。",
        redeemCode: "兑换码",
        redeemCodeDesc: "使用兑换码获取额外积分",
        enterCodePlaceholder: "请输入兑换码 (XXXX-XXXX-XXXX)",
        redeem: "兑换",
        proUser: "PRO 用户",
        proFeaturesUnlocked: "您已解锁高级功能。享受标准优先级和专属访问权限。",
        getMore: "获取更多积分 / Pro",
        adminTools: "管理员工具",
        hideAdminTools: "隐藏管理工具"
    },
    contextMenu: {
        copyContent: "复制内容",
        favorite: "收藏",
        unfavorite: "取消收藏",
        aiExpand: "AI 扩展",
        createConnection: "创建连接",
        delete: "删除",
        newCard: "新建卡片",
        newNote: "新建便签",
        paste: "粘贴",
        deleteConnection: "删除连线",
        lineColor: "线条颜色"
    },
    chatBar: {
        startNewBoard: "开始新画板...",
        askAboutSelected: "询问关于 {count} 个选中项...",
        typeToCreate: "输入以创建或提问...",
        uploadImage: "上传图片",
        addStickyNote: "添加便签",
        expandTopics: "扩展标记的主题",
        topics: "主题",
        selectConnected: "选择连接的集群",
        createZone: "创建区域（分组）",
        gridLayout: "网格布局",
        appendToChat: "追加到选中卡片的对话"
    },
    gallery: {
        gallery: "画廊",
        favorites: "收藏",
        trash: "回收站",
        usageGuide: "💡 使用指南",
        newBoard: "新建画板",
        signIn: "登录",
        pricing: "升级套餐",
        signOut: "退出登录",
        greetingMorning: "早上好",
        greetingAfternoon: "下午好",
        greetingEvening: "晚上好",
        creator: "创作者",
        readyToCreate: "准备好捕捉下一个灵感了吗?",
        recentlyVisited: "最近访问",
        allBoards: "所有画板",
        account: "账户",
        whatToCreate: "你想创造什么?",
        start: "开始",
        releaseToUpload: "松开以上传",
        recycleBin: "回收站",
        trashHint: "删除超过30天的项目将被永久移除",
        emptyTrash: "回收站为空",
        emptyTrashHint: "这里的项目可以被恢复。",
        freshStart: "全新开始",
        freshStartHint: "你的画布正在等待。在上方创建你的第一个画板。"
    },
    welcome: {
        badge: "全新思维方式",
        title: "无限画布 × AI 思维协作",
        subtitle1: "打破传统线性对话，在广阔的二维空间中自由挥洒灵感。",
        subtitle2: "随时召唤 Gemini 3.0 Pro & Flash，让每个节点都充满智慧。",
        aiChat: "AI 智能对话",
        aiChatDesc: "集成 Gemini 3.0 Pro & Flash，联网搜索、视觉识别、极速响应，一切尽在掌握。",
        infiniteCanvas: "无限画布",
        infiniteCanvasDesc: "双指随意缩放、无限拖拽平移。你的思维有多大，这片画布就有多大。",
        smartConnections: "智能连线",
        smartConnectionsDesc: "点击连接，AI 瞬间理解上下文。免去繁琐说明，让逻辑自然流淌。",
        autoLayout: "一键自动布局",
        autoLayoutDesc: "乱了？一键整理。思维导图算法，瞬间还原清晰树状结构。",
        batchOperations: "批量操作",
        batchOperationsDesc: "框选多张卡片，批量重写、删除或建立连接。效率提升不止十倍。",
        cloudSync: "云端同步",
        cloudSyncDesc: "基于 Firebase 实时同步。手机上的灵感，电脑上瞬间可见。",
        startCreating: "开始创作",
        firstVisitOnly: "💡 仅首次访问显示 · 点击按钮进入画布"
    },
    search: {
        placeholder: "搜索卡片、画板...",
        noResults: "没有找到匹配结果",
        enterKeywords: "输入关键词搜索所有画板",
        openSearch: "打开搜索",
        board: "画板",
        cards: "卡片",
        navigate: "导航",
        open: "打开",
        inBoard: "在"
    },
    chat: {
        conversation: "对话",
        neuralReader: "神经阅读器",
        neuralNotepad: "神经笔记本",
        capturedInsight: "已捕获的洞见",
        clearMarks: "清除标记",
        thinking: "思考中...",
        sproutIdeas: "发芽灵感",
        quickSprout: "发芽",
        branch: "分支",
        captureAsNote: "捕获为便签",
        markTopic: "标记主题",
        refineNote: "请 AI 完善这条笔记...",
        refineThought: "完善这个想法...",
        insightArchive: "洞见档案"
    },
    card: {
        newConversation: "新对话",
        copyResponse: "复制回复",
        createConnection: "创建连接",
        expand: "展开",
        noMessagesYet: "暂无消息",
        thinking: "思考中...",
        messages: "条消息",
        generating: "生成中...",
        noImage: "暂无图片"
    },
    common: {
        cancel: "取消",
        save: "保存",
        delete: "删除",
        confirm: "确认",
        close: "关闭",
        loading: "加载中...",
        error: "错误",
        success: "成功",
        undo: "撤销"
    },
    feedback: {
        title: "反馈",
        submitFeedback: "发送",
        placeholder: "分享你的想法... (回车发送)",
        emailHint: "支持 Gmail, QQ, Outlook, 163, iCloud",
        noFeedback: "暂无反馈，来抢沙发吧！",
        invalidEmail: "请使用有效的邮箱 (Gmail, QQ, Outlook, 163, iCloud)",
        hot: "热门",
        top: "最高票",
        recent: "最新",
        inProgress: "进行中",
        planned: "已计划",
        done: "已完成",
        pending: "待处理",
        votes: "票",
        comments: "评论",
        loginToVote: "登录后可投票",
        voteLimitReached: "已达投票上限 (1)",
        votesRemaining: "剩余投票数",
        upvote: "点赞",
        removeVote: "取消点赞"
    },
    pricing: {
        backToNexMap: "返回 NexMap",
        securedByStripe: "Stripe 安全支付",
        tagline: "简单透明，拒绝套路",
        heroTitle: "释放空间 AI 的无限潜能",
        heroDesc: "Pro 版解锁无限可能，积分包灵活适配需求。选择最适合你的方式开始。",
        creditPacks: "积分流量包",
        creditPacksDesc: "适合轻量级使用者。随用随充，灵活便捷。",
        proLifetime: "Pro 终身版",
        mostPopular: "最受欢迎",
        bestValue: "超值首选",
        credits: "积分",
        conversations: "次对话",
        getStarted: "立即购买",
        redirecting: "安全跳转中...",
        upgradeNow: "立即升级",
        oneTimePayment: "一次性买断",
        cancelAnytime: "无隐形消费，100% 安全保障。",
        proTitle: "Pro 终身版",
        proDesc: "为专业思考者打造的终极形态。拥有完整的数据主权，通过 BYOK (Bring Your Own Key) 打破所有限制，享受极致纯粹的空间思考体验。",
        proFeatures: [
            "无限空间画布 — 彻底跳出线性对话，在广阔的二维空间中自由构建思维宫殿",
            "多线程 AI 协同 — 拒绝等待，同时指挥 10+ 个 AI 智能体并行工作",
            "Ghost 极速流式 — 体验 <50ms 的极致低延迟交互，让 AI 跟上你的思维速度",
            "BYOK 自带密钥 — 直连 OpenAI/Anthropic/Google 官方接口，隐私绝对安全",
            "极致渲染性能 — 专为复杂项目优化，流畅支撑 500+ 卡片同屏协作",
            "终身免费更新 & 商业使用授权 — 一次付费，永久拥有"
        ],
        ssl: "256位 SSL 银行级加密",
        poweredByStripe: "由 Stripe 提供支付支持",
        instantDelivery: "支付成功即刻生效",
        allRights: "保留所有权利。",
        regionBlocked: "您所在的地区暂不支持支付服务",
        regionBlockedDesc: "应监管要求，中国大陆地区暂时无法使用在线支付功能。",
        contactUs: "联系客服",
        payAsYouGo: "按需付费，用多少充多少",
        noExpiration: "积分永久有效",
        bestValuePerCredit: "最佳性价比之选",
        prioritySupport: "优先技术支持",
        forPowerUsers: "重度用户推荐",
        maxEfficiency: "效率最大化",
        whyPro: "为什么选择 Pro？",
        whyProDesc: "如果你追求无限的画布空间、绝对的数据隐私和极致的响应速度，Pro 是你的不二之选。",
        casualUser: "免费开始，按需升级",
        casualUserDesc: "每位用户都享有慷慨的免费额度。需要更多时再充值。",
        freeTierBadge: "永久免费",
        freeTierTitle: "免费层级",
        freeTierDesc: "无需信用卡。每周一重置。",
        chatsPerWeek: "对话/周",
        imagesPerWeek: "图片/周",
        boards: "画板",
        hiddenPrice: "---",
        privacy: "隐私政策",
        terms: "服务条款",
        ownKey: "自带 Key",
        ownKey: "自带 Key",
        unlimited: "无限使用",
        getMorePower: "获取更多算力",
        chatPacks: "对话流量包",
        needMore: "需要更多？请购买下方的流量包",
        starter: "入门版",
        standard: "标准版",
        power: "专业版",
        becomePro: "成为 Pro 用户",
        unlockPotential: "一次性付费，解锁 NexMap 的全部潜能。",
        processing: "处理中...",
        loginRequired: "请先登录后再进行购买。",
        youGet: "您获得",
        perWeek: "每周",
        resetsMonday: "每周一重置",
        images: "张图片"
    },
    footer: {
        title: "开始用「连接」思考。",
        cta: "启动 Alpha 版",
        rights: "© 2024 NexMap. 保留所有权利。"
    },
    payment: {
        successTitle: "支付成功！",
        successDesc: "感谢您的购买，积分已添加到您的账户。",
        orderNumber: "订单编号",
        copy: "复制",
        copied: "已复制！",
        product: "产品",
        amount: "金额",
        creditsAdded: "已添加积分",
        proUnlocked: "已解锁 Pro 会员！",
        welcomePro: "欢迎成为 Pro！",
        contactSupport: "有疑问？联系我们："
    },
    stats: {
        title: "创意旅程",
        totalBoards: "创意宇宙",
        activeBoards: "活跃项目",
        trashBoards: "已归档",
        totalCards: "捕捉的灵感",
        cardsSubtitle: "跨越所有画布",
        aiCredits: "神经能量",
        creditsRemaining: "剩余",
        creditsUsed: "已消耗",
        globalTokens: "上下文上下文吞吐",
        lastActive: "上次头脑风暴",
        never: "从未",
        signIn: "登录以查看神经统计数据",
        loading: "正在加载神经指标...",
        dailyActivity: "神经突触活动",
        today: "今日生成",
        yesterday: "昨日",
        globalChars: "总生成字符"
    },
    // ===== TOOLBAR TRANSLATIONS =====
    toolbar: {
        items: "个项目",
        retry: "重试",
        sprout: "发芽",
        zone: "分区",
        delete: "删除",
        expand: "展开"
    },
    favorites: {
        collection: "收藏集",
        noFavorites: "暂无收藏",
        noFavoritesDesc: "将重要的对话收藏在这里，随时查看。",
        note: "笔记",
        shareTooltip: "分享为图片",
        removeTooltip: "取消收藏",
        from: "来自",
        goToSource: "前往来源看板",
        favoriteNoteTitle: "精选笔记",
        share: "分享",
        source: "来源",
        goToBoard: "查看原文"
    },
    zone: {
        dragToMove: "拖拽标题移动分区",
        description: "备注",
        descriptionPlaceholder: "添加备注...",
        icon: "图标",
        customColor: "自定义颜色",
        cardCount: "{count} 张卡片",
        messageCount: "{count} 条对话",
        colorPresets: "预设颜色",
        selectEmoji: "选择图标"
    },
    about: {
        backHome: "返回首页",
        manifesto: "我们的宣言",
        title: "超越对话框。",
        subtitle: "我们坚信，当前 LLM 的交互界面——那个狭窄的对话框——正在扼杀人类的创造力。",
        builtFor: "致敬叛逆者。",
        s1: {
            title: "认知囚笼",
            text: "看看现在的 AI 交互：把全人类最顶尖的智慧，囚禁在一个线性的文本终端里。这就像是用电报机在和爱因斯坦对话。我们被迫把多维度的思考压扁成一维的文字流。但这绝不是人类大脑的工作方式。我们是用连接思考的，是跳跃的，是在混乱的灵感网络中寻找秩序。"
        },
        s2: {
            title: "打破线性之墙",
            text: "MixBoard 不仅仅是一块画布；它是对现代 AI 线性界面的叛逆。我们在构建一个空间，让你的思想可以爆炸、分叉、重连。在这里，AI 的回答不是对话的终点，而是一片新森林的种子。我们要给 LLM 一个生存的空间维度，释放它的真正潜能。"
        },
        s3: {
            title: "未来是无限的",
            text: "这仅仅是个开始。未来的 AI 交互不再是为了找一行代码而翻阅 500 条历史记录。它将是一张即使在休眠时也活着的、会呼吸的智慧导图，由一个理解上下文、结构和意图的 AI 增强。我们在为下一代的思考者、创造者和梦想家打造工具。",
            extra: "欢迎来到无限画布。"
        }
    },
    aboutPage: {
        backToHome: "返回首页",
        manifesto: "我们的宣言",
        title: "超越对话框。",
        subtitle: "我们相信，目前的大型语言模型界面——对话框——是人类创造力的瓶颈。",
        section1: {
            title: "认知的囚笼",
            text: "看看我们如何与人类创造的最强大的智能进行互动。它被限制在一个线性的、基于文本的终端中。这感觉就像通过电报线与天才交谈。我们被迫将多维度的思想压缩成单一维度的文本流。这不是我们大脑的工作方式。我们在连接中思考，在空间中思考，在相关思想的混沌网络中思考。"
        },
        section2: {
            title: "打破线性之墙",
            text: "MixBoard 不仅仅是一个画布；它是对现代 AI 界面线性特征的反叛。我们正在建立一个空间，即使你的思想爆炸、分支并重新连接。在这里，AI 的回应不是对话的结束，而是一片新森林的种子。我们希望通过给 LLM 一个生存的空间维度，来释放它们的真正潜力。"
        },
        section3: {
            title: "未来是无限的",
            text: "我们才刚刚开始。AI 互动的未来不会是为了找到那一个代码片段而要在 500 条消息的历史记录中滚动。它将是一个活生生的、会呼吸的智力地图，由一个理解上下文、结构和意图的 AI 增强。我们正在为下一代思想家、创造者和梦想家构建工具。",
            extra: "欢迎来到无限画布。"
        },
        footer: "为反叛者而建。"
    },
    historyPage: {
        seoTitle: "更新日志",
        seoDesc: "查看 NexMap 的最新动态。我们开发历程的透明展示。",
        backToHome: "返回首页",
        label: "更新日志",
        titlePrefix: "演变",
        titleHighlight: "NexMap",
        subtitle: "透明地展示我们从粗糙的原型到空间思维引擎的旅程。我们快速交付并经常迭代。",
        endOfRecords: "记录结束。旅程继续。",
        changes: [
            {
                date: "2026年1月3日",
                version: "0.0.26",
                title: "SEO 与无障碍",
                items: [
                    "实施了基于页面的动态 SEO 元标签 (react-helmet-async)",
                    "添加了正确的 favicon 以用于 Google 搜索显示",
                    "修复了用户菜单按钮无法点击的问题 (CSS 堆叠上下文)",
                    "为免费用户添加了免费图像生成 (20张/周)",
                    "将免费层对话模型切换为 Kimi-K2-Thinking"
                ]
            },
            {
                date: "2026年1月3日",
                version: "0.0.25",
                title: "国际化与同步",
                items: [
                    "完整的 4 语言支持 (EN, ZH, JA, KO)",
                    "更新了应用程序图标为新的 NexMap 思维导图徽标",
                    "修复了画板提示词 (标签) 云同步问题",
                    "修复了 Sidebar 中的 Mac 输入法定位问题"
                ]
            },
            {
                date: "2026年1月3日",
                version: "0.0.24",
                title: "侧边栏与区域",
                items: [
                    "重新设计侧边栏为悬浮的 Notion 风格标签",
                    "添加区域 (Zone) 按钮以便快速进行卡片分组",
                    "实施了支持拖放的提示词侧边栏",
                    "修复了 BoardPage 中的无限循环错误"
                ]
            },
            {
                date: "2025年12月31日",
                version: "0.0.23",
                title: "发芽与分支",
                items: [
                    "添加了快速发芽 (Sprout) 功能，用于一键主题分解",
                    "实施了 '好奇心最大化' 提示策略",
                    "为发芽/分支提供了标准的思维导图布局",
                    "添加了新卡片放置的碰撞检测"
                ]
            },
            {
                date: "2025年12月31日",
                version: "0.0.22",
                title: "图像与导出",
                items: [
                    "使用插图屋风格提示改进了图像生成",
                    "修复了 iPad Safari 图像导出问题",
                    "添加了用户自定义指令支持",
                    "模块化 Gemini 提供程序以更好地处理错误"
                ]
            },
            {
                date: "2025年12月30日",
                version: "0.0.21",
                title: "画廊现代化",
                items: [
                    "使用玻璃拟态更新了画廊 UI",
                    "解决了移动端的滚动问题",
                    "添加了 API 错误的自动重试 (最多 2 次)",
                    "通过设置启用了手动语言切换"
                ]
            },
            {
                date: "2025年12月29日",
                version: "0.0.19",
                title: "AI 管理器",
                items: [
                    "修复了 Gemini 400 验证错误",
                    "解决了 AI 管理器消息丢失问题",
                    "清理图像数据以兼容 API"
                ]
            },
            {
                date: "2025年12月28日",
                version: "0.0.17",
                title: "安全与备份",
                items: [
                    "添加了 5 天定时同步回滚功能",
                    "修复了登录时数据清除的错误",
                    "引入了注销安全锁",
                    "使手动 JSON 导入始终可见以便恢复"
                ]
            },
            {
                date: "2025年12月28日",
                version: "0.0.16",
                title: "本地化",
                items: [
                    "本地化定价页面内容 (EN, ZH, JA)",
                    "添加了特定商交易法和隐私政策页面",
                    "实施了基于区域的访问限制"
                ]
            },
            {
                date: "2025年12月27日",
                version: "0.0.15",
                title: "稳定性",
                items: [
                    "修复了幽灵注销问题",
                    "阻止欢迎弹窗再次出现",
                    "优化代码结构"
                ]
            },
            {
                date: "2025年11月",
                version: "0.0.1",
                title: "初始发布",
                items: [
                    "基本的无限画布功能",
                    "实时 Google Firebase 同步",
                    "卡片中的 Markdown 支持",
                    "多模态 AI 聊天集成"
                ]
            }
        ]
    }
};

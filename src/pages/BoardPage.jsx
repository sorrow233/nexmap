...
export default function BoardPage({ user, boardsList, onUpdateBoardTitle, onUpdateBoardMetadata, onBack }) {
    ...
    useEffect(() => {
        ...
        const activeCardsCount = cards.filter(c => !c.deletedAt).length;
        if (currentBoardId && activeCardsCount > 10 && !hasAutoGeneratedRef.current && !currentBoard?.backgroundImage) {
            // Only auto-generate if no background exists
            if (!currentBoard?.backgroundImage) {
                console.log('Auto-generating background for board with > 10 cards...');
    ...
    }, [cards, currentBoardId, onUpdateBoardMetadata]);

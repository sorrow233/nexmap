...
export default function BoardPage({ user, boardsList, onUpdateBoardTitle, onUpdateBoardMetadata, onBack }) {
    ...
    const { generateBackground, generatingBoardId } = useBoardBackground();
    const hasAutoGeneratedRef = useRef(false);

    // Auto-generate background when active cards > 10
    useEffect(() => {
        // Filter out soft-deleted cards for this check
        const activeCards = cards.filter(c => !c.deletedAt);
        const activeCardsCount = activeCards.length;

        // Debug logging to understand why it triggers
        // console.log('[AutoGen Debug] Board Check:', { id: currentBoardId, count: activeCardsCount, hasBg: !!currentBoard?.backgroundImage });

        if (currentBoardId && activeCardsCount > 10 && !hasAutoGeneratedRef.current && !currentBoard?.backgroundImage) {
            console.log(`[AutoGen] Triggering! activeCards: ${activeCardsCount} > 10`);

            // Double check: Only trigger if we haven't done it this session
            // And ensure we have enough real content (e.g. checking length of text)
            // For now, restoring user's logic exactly but with the log.

            generateBackground(currentBoardId, (id, updates) => {
                if (onUpdateBoardMetadata) {
                    onUpdateBoardMetadata(id, updates);
                }
            }, { summaryOnly: false });
            hasAutoGeneratedRef.current = true;
        }
    }, [cards, currentBoardId, onUpdateBoardMetadata, currentBoard?.backgroundImage]);
